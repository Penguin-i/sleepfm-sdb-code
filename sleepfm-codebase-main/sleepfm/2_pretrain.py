...
from config import (CONFIG, CHANNEL_DATA, 
                    ...
                    PATH_TO_PROCESSED_DATA, LABELS_DICT)
...
@click.option("--hard_mining", type=bool, default=True, help="Enable hard negative mining for difficult samples")
@click.option("--adaptive_temp", type=bool, default=True, help="Enable adaptive temperature adjustment")
@click.option("--temp_init", type=float, default=0.07, help="Initial temperature value")
@click.option("--hard_mining_factor", type=float, default=2.0, help="Weight factor for hard negatives")
def train(
    dataset_dir,
    dataset_file,
    batch_size,
    ...
    if hard_mining:
        output += "_hard_mining"
    if adaptive_temp:
        output += "_adaptive_temp"

    output = os.path.join(CONFIG.OUTPUT, output)
    os.makedirs(output, exist_ok=True)
    
    # 使用指定的初始温度值
    temperature = torch.nn.parameter.Parameter(torch.as_tensor(temp_init))
    ...
    logger.info(f"Hard Mining: {hard_mining}, Hard Mining Factor: {hard_mining_factor}")
    logger.info(f"Adaptive Temperature: {adaptive_temp}, Initial Temperature: {temp_init}")
                ...
                with torch.set_grad_enabled(split == "pretrain"):
                    with tqdm.tqdm(total=len(dataloader)) as pbar:
                        for (resp, sleep, ekg) in dataloader:
                            # 获取当前批次的睡眠阶段标签（如果数据集中有这个信息）
                            sleep_stages = None
                            try:
                                # 尝试从dataset中获取睡眠阶段标签
                                if hasattr(dataset[split], 'get_sleep_stages') and callable(getattr(dataset[split], 'get_sleep_stages')):
                                    sleep_stages = dataset[split].get_sleep_stages(count * batch_size, (count + 1) * batch_size)
                            except:
                                pass
                            ...
                            if mode == "pairwise":
                                ...
                                for i in range(num_targets):
                                    for j in range(i + 1, num_targets):
                                        ...
                                        # 难样本挖掘：计算每个样本的损失，并增加困难样本的权重
                                        if hard_mining and split == "pretrain":
                                            per_sample_loss = torch.nn.functional.cross_entropy(
                                                logits, labels, reduction="none")
                                            
                                            # 根据损失值识别困难样本（损失大的样本是难样本）
                                            hard_indices = per_sample_loss > torch.median(per_sample_loss)
                                            
                                            # 为困难样本增加权重
                                            weights = torch.ones_like(per_sample_loss)
                                            weights[hard_indices] = hard_mining_factor
                                            
                                            l = (per_sample_loss * weights).sum()
                                        else:
                                        ...
                                        if hard_mining and split == "pretrain":
                                            per_sample_loss = torch.nn.functional.cross_entropy(
                                                logits.transpose(0, 1), labels, reduction="none")
                            ...
                            if adaptive_temp and split == "pretrain":
                                # 根据当前批次准确率自适应调整温度
                                if mode == "pairwise":
                                    accuracy = np.mean([total_correct[i, j] / total_pairs[i, j] for i in range(len(modality_types)) for j in range(i + 1, len(modality_types))])
                                else:  # leave_one_out
                                    accuracy = np.mean([total_correct[i, j] / total_pairs[i, j] for i in range(len(modality_types)) for j in [0, 1]])
                                
                                # 如果准确率过高，增加温度使任务更困难；如果准确率过低，降低温度使任务更简单
                                with torch.no_grad():
                                    if accuracy > 0.9:
                                        temperature.mul_(1.01)  # 增加温度，使任务更难
                                    elif accuracy < 0.6:
                                        temperature.mul_(0.99)  # 降低温度，使任务更容易
